...
declare module ".." {
  interface ColorMaster {
     ...
     * @example CM({ r: 200, g: 150, b: 100, a: 0.7 }).stringHWB() → "xyza(37, 35, 17, 0.7)"
...
const XYZPlugin: TPlugin = (CM): void => {
  ...
  CM.prototype.stringXYZ = function ({ alpha = true, precision = [0, 0, 0, 1] as TNumArr } = {}): string {
    const [x, y, z, a] = Object.values(this.xyza() as Ixyza).map((val, i) => round(val, precision[i] ?? 1));
